...
async function run(paymentMethod,payload){
 const element=()=>({children:[],hidden:false,textContent:'',innerHTML:'',append(...items){this.children.push(...items)},scrollIntoView(options){this.scroll=options}});
 const status=element(),instructions=element(),receipt=element(),button=element(),result=element();
 result.hidden=true;result.querySelector=s=>s==='[data-payment-instructions]'?instructions:receipt;
 ...
 if(paymentMethod==='ARS_GALICIA'&&payload.quote){
  const block=instructions.children[0];
 ...
 }else if(paymentMethod==='PAYPAL')assert(instructions.children[0].textContent.endsWith('USD 157.50'));
